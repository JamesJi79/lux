defmodule Lux.Integrations.Binance do
  @moduledoc "Binance exchange integration for spot trading and market data."
  ...
  def market_order(symbol, side, qty), do: signed_post("/api/v3/order", %{symbol: String.upcase(symbol), side: String.upcase(side), type: "MARKET", quantity: qty})
  def limit_order(symbol, side, qty, price), do: signed_post("/api/v3/order", %{symbol: String.upcase(symbol), side: String.upcase(side), type: "LIMIT", timeInForce: "GTC", quantity: qty, price: price})
  ...
  defp request(method, path, params, signed) do
    client = Application.get_env(:lux, :binance_client, Req)
    params = if signed, do: sign_params(params), else: params
    url = @base_url <> path <> (if params != %{}, do: "?" <> URI.encode_query(params), else: "")
    headers = [{"Content-Type", "application/json"}]
    headers = if signed, do: [{"X-MBX-APIKEY", api_key()} | headers], else: headers

    case client.request(method, url, headers, []) do
  ...
  defp api_key, do: Application.get_env(:lux, :binance_api_key) || ""
  defp secret_key, do: Application.get_env(:lux, :binance_secret_key) || ""
